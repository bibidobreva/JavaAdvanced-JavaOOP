...
public class Main {
    public static void main(String[] args) {
        ...
        Set<Field> fields = Arrays.stream(clazz.getDeclaredFields()).collect(Collectors.toCollection(() -> new TreeSet<>(fieldComparator)));
        ...
        for (Method method : methods) {
            if (method.toString().contains("get")) {
                getters.add(method);
            } else if (method.toString().contains("set")) {
                setters.add(method);
            }
